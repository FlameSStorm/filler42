...
int is_fit(t_meta *meta, int x, int y)
{
    ...
    while (++i < meta->piece.size_x)
    {
        ...
        {
            if (meta->piece.piece[i][j] == '*' && (meta->map[x + i][y + j] == meta->player || meta->map[x + i][y + j] == meta->player + 32))
                    is_on++;
            if (meta->piece.piece[i][j] == '*' && (meta->map[x + i][y + j] == meta->enemy ||
...
void   find_option(t_meta *meta)
{
    ...
    while (i < meta->size_x - meta->piece.size_x)
    {
        ...
        while (j < meta->size_y  - meta->piece.size_y)
        {
            if (is_fit(meta, i, j))
            {
                option[k].distance = ABS(i - meta->enemy_x) + ABS(j - meta->enemy_y);
                option[k].x = i;
                option[k++].y = j;
            }
            j++;
